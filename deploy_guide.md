# Guía de Despliegue Gratuito - Intelectia

Esta guía explica cómo desplegar el backend de Intelectia (API + PostgreSQL) en servicios free-tier.

> ⚠️ **Nota:** El cliente WPF es una aplicación de escritorio Windows y se distribuye por separado (ej. GitHub Releases). Este despliegue es exclusivamente para el backend API.
...
1. Crea un proyecto nuevo y añade un servicio **PostgreSQL** (Free Tier: $5 de crédito mensual).
...
|------------|---------|------------|
| **Inactividad (Render)** | El servicio se suspende tras 90 días sin tráfico | Usa un ping externo gratuito (ej. UptimeRobot) cada 15 min para mantenerlo activo |
